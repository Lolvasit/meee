...
from aiogram.types import InputFile, Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ContentType, \
    ReplyKeyboardMarkup, KeyboardButton, MessageEntity, Chat
...
async def join_request_handler(join_request: types.ChatJoinRequest):
    ...
    try:
        if not setting.captcha_after:
            ...
            await send_all_messages(bot, user_id, title, send_captcha, *[bot, user_id, user])
...
async def _process_mail(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    if send_date is not None:
        ...
        job = scheduler.add_job(mails.make_mail, trigger=DateTrigger(date), args=(
...
def format_text(message: Message, user: Chat | User, title=None) -> typing.Dict[str, typing.Any]:
    if message is None:
    ...
    text = message.text if not (message.text is None) else message.caption

    entities = list()
    search_str = ["{id}", "{username}", "{fname}", "{lname}", "{fullname}", "{ctitle}", "{anyname}"]
    ...
    replace_data = [
        ...
        user.full_name,
        title,
        user.full_name if user.username is None else user.username
    ]
    positions_before = list()

    for i in range(len(search_str)):
        for item in [match.start() for match in re.finditer(search_str[i], text)]:
            obj = {"position": item, 'length': len(search_str[i]), "token": search_str[i]}
            positions_before.append(obj)

    positions_before.sort(key=lambda x: x["position"])
    ...
    if message.entities is not None:
        for item in message.entities:
            entities.append(MessageEntity(
                ...
                item.offset,
                item.length,
                item.url if item.url is not None else None,
            ))
    ...
    for i in range(len(positions_before)):
        obj = positions_before[i]
        data = replace_data[search_str.index(obj["token"])]
        text = text.replace(obj["token"], str(data), 1)
        delta = len(str(data)) - obj["length"]
        j = i
        while j < len(positions_before):
            positions_before[j]["position"] += delta
            j += 1

        for ent in entities:
            if ent.offset > (positions_before[i]["position"] + positions_before[i]["length"] - 1):
                ent.offset += delta
...
async def apply_or_decline(message: Message):
    ...
    if message.text in json.loads(setting.captcha_buttons):
        try:
            ...
            current_loop.create_task(bot.delete_message(user_id, message.message_id - 1))
        except Exception as e:
            print(e)
